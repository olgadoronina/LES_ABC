...
fig_height = 0.45*text_height*inches_per_pt       # height in inches
...
N_params = 4
path_base = '../ABC/final'
output_sigma = os.path.join(path_base, '4_params_sigma', 'output',)
output_prod = os.path.join(path_base, '4_params_prod', 'output',)
output_both = os.path.join(path_base, '4_params_both', 'output',)
...
for i in range(N_params):
    ...
    C_limits_both[i] = [np.min(accepted_both[:, i]), np.max(accepted_both[:, i])]
C_limits_sigma[0, 1] = np.min([0.0, C_limits_sigma[0, 1]])
C_limits_prod[0, 1] = np.min([0.0, C_limits_prod[0, 1]])
C_limits_both[0, 1] = np.min([0.0, C_limits_both[0, 1]])
print(C_limits_sigma)
print(C_limits_prod)

C_limits = np.zeros((N_params, 2))
for i in range(N_params):
    minimum = np.min([C_limits_sigma[i, 0], C_limits_prod[i, 0], C_limits_both[i, 0]])
    maximum = np.max([C_limits_sigma[i, 1], C_limits_prod[i, 1], C_limits_both[i, 1]])
...
for i in range(N_params):
    for j in range(N_params):
        if i == j:
            ...
            ax_diag[i].tick_params(axis='y', which='major', pad=0.2)
...
plt.colorbar(im_cond, cax=cax, ticks=ticks)
# format=ticker.FormatStrFormatter('%.1e'),

fig.subplots_adjust(left=0.2, right=0.8, wspace=0.11, hspace=0.11, bottom=0.08, top=0.98)
fig.savefig(os.path.join(path_base, 'conditional_combined_4'))
plt.close('all')