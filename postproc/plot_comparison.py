...
for ind, key in enumerate(['uu', 'uv', 'uw']):
    ...
    if os.path.isfile(os.path.join(path, '3_param_sigma')):
        ...
        axarr[0, ind].plot(x, y_max_joint[key], ':',  color=colors[1], linewidth=1, label='3 param sigma')
    if os.path.isfile(os.path.join(path, '4_param_sigma')):
        y_max_joint[key] = np.loadtxt(os.path.join(path, '4_param_sigma'))[ind]
        axarr[0, ind].plot(x, y_max_joint[key], ':',  color=colors[2], linewidth=1, label='4 param sigma')
    if os.path.isfile(os.path.join(path, '3_param_production')):
        ...
        axarr[1, ind].plot(x, y_max_joint[key], '--', color=colors[1], linewidth=1, label='3 param production')
    if os.path.isfile(os.path.join(path, '4_param_production')):
        ...
        axarr[1, ind].plot(x, y_max_joint[key], '--', color=colors[2], linewidth=1, label='4 param production')
    if os.path.isfile(os.path.join(path, '3_param_production')):
        y_max_joint[key] = np.loadtxt(os.path.join(path, '3_param_both'))[ind]
        axarr[2, ind].plot(x, y_max_joint[key], '--', color=colors[1], linewidth=1, label='3 param sigma+production')
    for i in range(3):
...
if os.path.isfile(os.path.join(path,  '3_param_sigma')):
    ...
    axarr[0, 3].plot(x, y_max_joint, ':',  color=colors[1], linewidth=1)
if os.path.isfile(os.path.join(path, '4_param_sigma')):
    y_max_joint = np.loadtxt(os.path.join(path, '4_param_sigma'))[3]
    axarr[0, 3].plot(x, y_max_joint, ':', color=colors[2], linewidth=1)
if os.path.isfile(os.path.join(path, '3_param_production')):
    y_max_joint = np.loadtxt(os.path.join(path, '3_param_production'))[3]
    axarr[1, 3].plot(x, y_max_joint, '--', color=colors[1], linewidth=1)
if os.path.isfile(os.path.join(path, '4_param_production')):
    y_max_joint = np.loadtxt(os.path.join(path, '4_param_production'))[3]
    axarr[1, 3].plot(x, y_max_joint, '--', color=colors[2], linewidth=1)
if os.path.isfile(os.path.join(path, '3_param_production')):
    y_max_joint = np.loadtxt(os.path.join(path, '3_param_both'))[3]
    axarr[2, 3].plot(x, y_max_joint, '--', color=colors[1], linewidth=1)
for i in range(3):
...
for i in range(4):
    axarr[2, i].set_xlabel(titles[i], labelpad=2)
for i in range(3):
    axarr[i, 0].set_ylabel('ln(pdf)', labelpad=2)

fig.subplots_adjust(left=0.09, right=0.98, wspace=0.1, bottom=0.12, top=0.9)
...
axarr[0, 1].legend(custom_lines, ['true data', '3 parameters', '4 parameters'], loc='upper center',
                   bbox_to_anchor=(0.94, 1.5), frameon=False,
