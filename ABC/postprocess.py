...
class PostprocessABC(object):
            ...
            for i in range(self.N.params):
                for j in range(self.N.params):
                    if i == j:
                        ...
                        max = x[np.argmax(y)]
                        print(self.params_names[i] + 'marginal mean and max', mean, max)
    ...
    def plot_compare_tau(self, scale='LES'):
        ...
        if scale == 'LES':
            ...
            if C_final_joint:
                tau_modeled_joint = model.NonlinearModel(g.LES, self.N, self.C_limits).Reynolds_stresses_from_C(
                    C_final_joint)
        ...
        if scale == 'TEST_M':
            ...
            if C_final_joint:
                tau_modeled_joint = model.NonlinearModel(g.TEST_sp, self.N, self.C_limits).Reynolds_stresses_from_C(
                    C_final_joint)
        ...
        if scale == 'TEST':
            titles = [r'$\widehat{\sigma}_{11}$', r'$\widehat{\sigma}_{12}$', r'$\widehat{\sigma}_{13}$']
            if C_final_joint:
                tau_modeled_joint = model.NonlinearModel(g.TEST, self.N, self.C_limits).Reynolds_stresses_from_C(
                    C_final_joint)
    ...
    def plot_eps(self):
        ...
        for ind, new_eps in enumerate(eps):
            ...
            logging.info('accepted {} values ({}%)'.format(
                len(g.accepted), round(len(g.accepted) / (self.N.each**self.N.params) * 100, 2)))
            for i in range(self.N.params):
                data = g.accepted[:, i]
