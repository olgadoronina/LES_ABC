...
import os
import sys
import teslacu.fft_mpi4py_numpy as tcfft        # FFT transforms and math functions
import teslacu.stats_mpi4py_numpy as tcstats    # statistical functions
import teslacu.findiff_numpy_scipy as tcfd      # finite difference functions
import teslacu.akima_numpy_scipy as tcas        # Akima spline approximation functions
...
class hitAnalyzer(mpiBaseAnalyzer):
    ...
    def vec_fft(self, var):
        ...
        nk = int(nx/2+1)
        nny = int(ny/self.comm.size)
        nz = int(nnz*self.comm.size)
